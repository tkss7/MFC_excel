...
DlgProxy.h, DlgProxy.cpp - 자동화 개체
    이 파일에는 CMFCApplication1DlgAutoProxy 클래스가 포함됩니다. 이 클래스는 자동화 컨트롤러가 대화 상자 엑세스에 사용할 수 있는 자동화 메서드 및 속성 노출을 처리하므로 대화 상자에 대한 자동화 프록시 클래스라고 합니다. 모달 대화 상자 기반 MFC 응용 프로그램의 경우 자동화 개체를 사용자 인터페이스에서 분리하기가 더욱 명확하고 쉽기 때문에 대화 상자 클래스에서 이 메서드 및 속성이 직접 노출될 수 없습니다.
